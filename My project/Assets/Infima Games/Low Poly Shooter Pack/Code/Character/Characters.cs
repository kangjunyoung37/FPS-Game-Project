...
namespace InfimaGames.LowPolyShooterPack
{
	...
	public sealed class Characters : CharacterBehaviour
	{
		...
		protected override void Update()
		{
			...
			aimingAlpha = characterAnimator.GetFloat(AHashess.AimingAlpha);
		...
		private void UpdateAnimator()
		{
			...
			characterAnimator.SetFloat(AHashess.LeaningForward, leaningValue, 0.5f, Time.deltaTime);
			...
			characterAnimator.SetFloat(AHashess.Movement, movementValue, dampTimeLocomotion, Time.deltaTime);
			
			//Aiming Speed Multiplier.
			characterAnimator.SetFloat(AHashess.AimingSpeedMultiplier, aimingSpeedMultiplier);
			
			//Turning Value. This determines how much of the turning animation to play based on our current look rotation.
			characterAnimator.SetFloat(AHashess.Turning, Mathf.Abs(axisLook.x), dampTimeTurning, Time.deltaTime);

			//Horizontal Movement Float.
			characterAnimator.SetFloat(AHashess.Horizontal, axisMovement.x, dampTimeLocomotion, Time.deltaTime);
			//Vertical Movement Float.
			characterAnimator.SetFloat(AHashess.Vertical, axisMovement.y, dampTimeLocomotion, Time.deltaTime);
			
			//Update the aiming value, but use interpolation. This makes sure that things like firing can transition properly.
			characterAnimator.SetFloat(AHashess.AimingAlpha, Convert.ToSingle(aiming), dampTimeAiming, Time.deltaTime);
			...
			characterAnimator.SetFloat(AHashess.PlayRateLocomotionForward, movementBehaviour.GetMultiplierForward(), 0.2f, Time.deltaTime);
			//Update Sideways Multiplier. This allows us to change the play rate of our animations based on our movement multipliers.
			characterAnimator.SetFloat(AHashess.PlayRateLocomotionSideways, movementBehaviour.GetMultiplierSideways(), 0.2f, Time.deltaTime);
			//Update Backwards Multiplier. This allows us to change the play rate of our animations based on our movement multipliers.
			characterAnimator.SetFloat(AHashess.PlayRateLocomotionBackwards, movementBehaviour.GetMultiplierBackwards(), 0.2f, Time.deltaTime);

			#endregion
			
			//Update Animator Aiming.
			characterAnimator.SetBool(AHashess.Aim, aiming);
			//Update Animator Running.
			characterAnimator.SetBool(AHashess.Running, running);
			//Update Animator Crouching.
			characterAnimator.SetBool(AHashess.Crouching, movementBehaviour.IsCrouching());
		}
		/// <summary>
		...
		private void PlayReloadAnimation()
		{
			...
			characterAnimator.SetBool(AHashess.Reloading, reloading = true);
		...
		{
			//Update.
			characterAnimator.SetBool(AHashess.Bolt, bolting = value);
		}
		/// <summary>
